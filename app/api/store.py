...
async def create_store(
    store: StoreCreate, store_ops: StoreOperations = Depends(get_store_operations)
):
    """Create a new store"""
...
async def get_store(
    store_id: str, store_ops: StoreOperations = Depends(get_store_operations)
):
    """Get store by ID"""
...
async def add_item_to_store(
    store_id: str,
    item: ItemCreate,
    store_ops: StoreOperations = Depends(get_store_operations),
):
...
async def add_multiple_items(
    ...
    if not success:
        raise HTTPException(
            status_code=404, detail="Store not found or items not added"
        )
...
async def update_store_conditions(
    store_id: str,
    conditions: StoreUpdate,
    store_ops: StoreOperations = Depends(get_store_operations),
):
